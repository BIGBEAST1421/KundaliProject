        ...
        <AnimatePresence mode="wait">
          {!ready ? (
            ...
            <motion.div key="chart" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.35 }} className={loading ? "opacity-70 transition-opacity" : ""}>
              <NorthChart lagna={data.lagna} d1={data.d1} title={t("sec_chart")} />
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                <dt className="text-muted">{t("lbl_lagna")}</dt><dd className="font-medium">{data.lagna}</dd>
                <dt className="text-muted">{t("lbl_moonsign")}</dt><dd className="font-medium">{data.rashi}</dd>
                <dt className="text-muted">{t("lbl_nakshatra")}</dt><dd className="font-medium">{data.nakshatra} · {t("pada")} {data.pada}</dd>
                <dt className="text-muted">{t("lbl_mahadasha")}</dt><dd className="font-medium">{data.mahadasha}</dd>
              </dl>
            </motion.div>
          ) : null}
        </AnimatePresence>
